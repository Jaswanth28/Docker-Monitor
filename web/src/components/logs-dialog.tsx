...
export function LogsPanel({ containerId, active = true, className }: { containerId: string | null; active?: boolean; className?: string }) {
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col gap-2", className)}>
      ...
      <pre ref={pre} className="bg-zinc-950 text-zinc-100 min-h-0 flex-1 overflow-auto rounded-md p-3 font-mono text-xs leading-5 whitespace-pre-wrap">
        {shown.join("\n") || "waiting for output…"}
      </pre>
    </div>
  )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[85vh] flex-col sm:max-w-5xl">
        ...
        <LogsPanel containerId={containerId} active={open} />
      </DialogContent>
    </Dialog>
  )
}
